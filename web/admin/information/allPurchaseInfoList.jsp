...
<html>
	...
	<link rel="stylesheet" rev="stylesheet" href="<%=path%>/css/comdo.css" type="text/css" />
		...
		function info(para)
		{
			document.location="<%=path%>/admin/info/toAllPurchaseInfoList?censor="+para;
		}
		function checkAll(){
		...
		function openSelectDiv(id){
			...
			var ret=butil.getUrl("<%=path%>/admin/info/checkPurchaseInfo?infoId="+id);
			var s=ret.split("}@");
	...
	<body>
		...
		<s:form action="toAllPurchaseInfoList" id="form1">
		<input id="curCensor" type="hidden" value="<s:property value="#request.censor"></s:property>" name="censor"/>
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 5px;">
		<tr>
			<td nowrap width="75px" align="right">会员ID号：</td>
			<td width="30"><input id="sid" type="text" name="memberId" size="20" value="<s:property value="#request.memberId"></s:property>"></td>
			<td nowrap width="80px" align="right">会员名称：</td>
			<td width="30"><input type="text" name="memberName" size="20" value="<s:property value="#request.memberName"></s:property>"></td>
			<td align="left">&nbsp;</td>
		</tr>
		<tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  align="center" class="list" >
				...
				<s:iterator id="item" value="listP" status="list"> 
					<tr class='<%=(looprow%2==0)?"row":"altRow"%>'>
						...
						<td class="zuo"><input type="button" value="预览" class="button2" onclick="window.open('${pageContext.request.contextPath}/front/info/showPurchaseInfo?infoId=<s:property value="#item.infoId"/>&type=<s:property value="#item.type"/>')"></input></td>
					</tr>
				<%
				looprow ++;
				%>
				...
				<tr>
					...
					<s:if test='#request.censor=="3"||#request.censor=="1"||#request.censor=="11" '>
						<input type="button" value="审查通过" class="button5" onclick="submitSelectBox('setPurchaseInfoCensor?infoIds=','doCensor=2')"></input>
						<input type="button" value="屏蔽（敏感词）" class="button5" onclick="submitSelectBox('setPurchaseInfoCensor?infoIds=','doCensor=21')"></input>
						<input type="button" value="屏蔽（其他问题）" class="button5" onclick="submitSelectBox('setPurchaseInfoCensor?infoIds=','doCensor=29')"></input>
						<input type="button" value="待查（其他问题）" class="button5" onclick="submitSelectBox('setPurchaseInfoCensor?infoIds=','doCensor=19')"></input>
					</s:if>
		...
		<div align="right" style="height: 50px; width: 792px; margin-top: 5px;" >
			<s:form action="toAllPurchaseInfoList">
			...
			<input type="hidden" value="<s:property value="#request.memberId"></s:property>" name="memberId"/>	
			<input type="hidden" value="<s:property value="#request.memberName"></s:property>" name="memberName"/>
			</s:form>&nbsp;&nbsp;
   		</div>
   		<br>
		...
		<table width="553" height="32" border="0" align="center" cellpadding="0" cellspacing="0">
		  <tr>
		    <td background="<%=path%>/image/comdo/tit_bg.gif"><table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
		      <tr>		        
		        <td height="32"width="90%" style="text-align:center; font-size:14px; font-weight:bold; color:#ffffff;"><div class="move">求购信息审核</div></td>
		...
		<table>
  				<tr>
					<td colspan="3">
					<s:if test='#request.censor=="3"||#request.censor=="1"||#request.censor=="11" '>
						<input type="button" value="审查通过" class="sm-submit4" onclick="submitInfo('setPurchaseInfoCensor?infoIds=','doCensor=2')"></input>
						<input type="button" value="屏蔽（敏感词）" class="sm-submit7" onclick="submitInfo('setPurchaseInfoCensor?infoIds=','doCensor=21')"></input>
						<input type="button" value="屏蔽（其他问题）" class="sm-submit8" onclick="submitInfo('setPurchaseInfoCensor?infoIds=','doCensor=29')"></input>
						<input type="button" value="待查（其他问题）" class="sm-submit8" onclick="submitInfo('setPurchaseInfoCensor?infoIds=','doCensor=19')"></input>
					</s:if>
					<s:elseif test='#request.censor=="19"'>
						<input type="button" value="审查通过" class="sm-submit4" onclick="submitInfo('setPurchaseInfoCensor?infoIds=','doCensor=2')"></input>
						<input type="button" value="屏蔽（敏感词）" class="sm-submit7" onclick="submitInfo('setPurchaseInfoCensor?infoIds=','doCensor=21')"></input>
