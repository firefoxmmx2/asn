...
<script type="text/javascript">	
	function inGroup()
	{
		...
		else
		{
			if(confirm('你确定对选定的会员进行分组?')){
				document.myform.action="<%=cp%>/do/client/inGroup";
				document.myform.submit();
				window.parent.myform.action="<%=cp%>/do/client/getGroupMember";
	...
	function sel()
	{
		document.myform.action="<%=cp%>/do/client/checkGroup";
		document.myform.submit();
	}
</script>
<style type="text/css">
...
<form  action="<%=cp%>/do/client/checkGroup" name="myform" id="myform" >
			...
			<table width="100%" class="table_border">
				...
				<tr>
				  <td class="color_h" style="font-size:12px">组名:	
						<input name="groupName" type="text" id="groupName" value="<%=request.getAttribute("groupName")==null?"":request.getAttribute("groupName")%>" size="25" /> 
