...
<body>
	...
	<br>
	<s:form action="/do/infoc/updateInfoCenter" method="post" id="form1" theme="simple">
		<input type="hidden" id="status" name="info.status" value="${info.status}"></input>
		<input type="hidden" name="info.id" value="${info.id}"></input>
		<input type="hidden" name="info.type" value="${info.type}"></input>
		<input type="hidden" name="info.createMan" value="${info.createMan}"></input>
		<table width="90%" align="center" cellpadding="0" cellspacing="0">
				<tr>
            		<td height="30" class="wz_left" ><span style="color:red;">*</span>信息标题：</td>
                	<td><input type="text" name="info.title" size="38" value="${info.title}"></input><span id="errorsDiv_info.title"></span></td>
            	</tr>	
            	<tr>
            		<td height="30" class="wz_left" ><span style="color:red;">*</span>信息正文：</td>
                	<td>
                	<textarea name="info.content" rows="15" cols="60">${info.content }</textarea>
                	<span id="errorsDiv_info.content"></span>
                	</td>
            	</tr>
            	<tr>
                 	<td height="40">&nbsp;</td>
	              	<td><input type="submit" value="确定" id="fb" />&nbsp;
	              	<s:if test='info.status!="Y"'>
	              	<input type="button" value="发布上网" onclick="JavaScript:document.getElementById('status').value='Y';document.getElementById('fb').click();"/>
	              	</s:if>
	              	</td>
	        	</tr>   
		</table>	
