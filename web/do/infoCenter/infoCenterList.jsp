...
<html>
	...
	<br />
		<s:form action="toInfoCenterList">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 5px;">
		<tr>
			<td nowrap width="70px" align="right">信息标题：</td>
			<td width="30"><input type="text" name="title" size="20" value="<s:property value="#request.title"/>"><input type="hidden" name="status" value="<s:property value="#request.status"/>"/></td>
			<td align="left"><input type="submit" value=" 查 询 " class="button5"><td>
			<td align="right">
			<input type="button" value="新增信息" id="add1"/>
			<td>
		</tr>
		</table>
		</s:form>
				<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
				  <s:if test='#request.status=="S"'>
				  <tr>
                      <td  height="33" width="20" background="../../image/hzx/change_bg.gif"></td>
                      <td width="116"  align="center" valign="middle" background="../../image/hzx/change1.gif" class="color_g2"><a href="JavaScript:;" onclick="doRedirect('toInfoCenterList?status=Y');return false;">已发布上网</a></td>
					  ...
					  <td width="6" background="../../image/hzx/change_bg.gif"></td>
                      <td width="116" align="center" valign="middle" background="../../image/hzx/change1.gif" class="color_g2"><a href="JavaScript:;" onclick="doRedirect('toInfoCenterList?status=N');return false;">草稿箱</a></td>
					  <td width="6" background="../../image/hzx/change_bg.gif"></td>
                      <td width="404" background="../../image/hzx/change_bg.gif">&nbsp;</td>
                  </tr>
				  </s:if>
				  <s:elseif test='#request.status=="N"'>
				  <tr>
                      <td  height="33" width="20" background="../../image/hzx/change_bg.gif"></td>
                      <td width="116"  align="center" valign="middle" background="../../image/hzx/change1.gif" class="color_g2"><a href="JavaScript:;" onclick="doRedirect('toInfoCenterList?status=Y');return false;">已发布上网</a></td>
					  <td width="6" background="../../image/hzx/change_bg.gif"></td>
                      <td width="116" align="center" valign="middle" background="../../image/hzx/change1.gif" class="color_g2"><a href="JavaScript:;" onclick="doRedirect('toInfoCenterList?status=S');return false;">已取消发布</a></td>
					  <td width="6" background="../../image/hzx/change_bg.gif"></td>
                  ...
                  <s:if test="infoList.size()<1">
                  <tr>
				    <td colspan="7" height="100" class="wz-18b"><img src="../../image/hzx/tous1.gif" /><span>暂时没有信息！</span></td>
				  </tr>
                  </s:if>
		</table>
		<s:if test="infoList.size()>=1">
		<div><input id="total" type="text" value='<s:property value="infoList.size()"/>' style="display: none;"></input></div>
		<div>
			<table width="100%" border="0" cellspacing="0" cellpadding="0"  align="center" class="list" >
				<tr class="head" style="text-align: center;font: 14px solid bold;">
					...
					<td class="libiaobg" nowrap width="120">信息标题</td>
					<td class="libiaobg" nowrap width="56">创建时间</td>
				...
				<s:iterator id="item" value="infoList" status="list"> 
					<tr class='<%=(looprow%2==0)?"row":"altRow"%>' height="50">
						...
						<td class="zuo" style="text-align: center;"><a target="_blank" href="<%=path%>/front/infoc/showInfoCenter?id=<s:property value="#item.id" />"><s:property value="#item.title"></s:property></a></td>
						<td class="zuo" style="text-align: center;"><s:date name="#item.createTime" format="yyyy-MM-dd"></s:date></td>
						<td class="zuo" align="center">
						<a href="toUpdateInfoCenter?id=<s:property value="#item.id" />">修改</a>
						</td>
				...
				<tr>
					...
					&nbsp;&nbsp;
					<s:if test='#request.status=="S"'>
					<input type="button" value="重新发布" class="button5" onclick="submitSelectBox1('setInfoCenterStatus','Y')"></input>
					<input type="button" value="删除" class="button5" onclick="delSelectBox('deleteInfoCenter','S')"></input>
					</s:if>
					<s:elseif test='#request.status=="N"'>
					<input type="button" value="发布上网" class="button5" onclick="submitSelectBox1('setInfoCenterStatus','Y')"></input>
					<input type="button" value="删除" class="button5" onclick="delSelectBox('deleteInfoCenter','N')"></input>
					</s:elseif>
					<s:else>
					<input type="button" value="取消发布" class="button5" onclick="submitSelectBox('setInfoCenterStatus','S')"></input>
					</s:else>
					</td>
					<td colspan="1"></td>
				</tr>
		...
		<div align="right" width="98%">
			<s:form action="toInfoCenterList">
				...
				<input type="hidden" name="title" value="<s:property value="#request.title"/>"/>
