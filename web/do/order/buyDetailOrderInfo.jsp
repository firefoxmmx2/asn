...
<%
	String pubSvrUrl = "http://pubsvr.aiecp.com/asn";
  	...
  	Member member=  MemberService.getMember(info.getSupplierId());
...
<HEAD>
    <TITLE>订单信息(卖家)</TITLE>
	<LINK href="/asn/do/order/css/orderdetail.css" type=text/css rel=stylesheet>
...
<DIV id=tabPanel>
    ...
    <DIV class=tab-context>
    	...
    	<DIV class=box>          
	        <H4><font color="#404040">卖家信息</font></H4>
	           <TABLE><!--  class=seller -->
	               <TBODY>
	                   <TR class=first>
	                      <td>卖家名称：<A href="<%=request.getContextPath()%>/front/shop/supplierShopSy?supplierid=<%=member.getMemberId()%>" target="_blank"><%=member.getFullName() %></A>
           ...
           <div class=box>
         	...
         	<!-- 	
                                       订&nbsp;单号：<s:property value="order.id"/>  
               <font color="red">                                
	   ...
	   <DIV class=deal-wrap>
		...
		<DIV class=split></DIV>
		<DIV class=box>
			<H4><font color="#404040">卖家留言</font></H4>
			<P>
				<s:if test="order.leaveWord == null">卖家没任何回复。</s:if>
				<s:else><s:property value="order.leaveWord"/></s:else>
			</P>
	    </DIV>
	    <!-- 
		<DIV class=split></DIV>
	    ...
	    <DIV class=split></DIV>
	    <DIV class=box>
	          如需再次购买订单中的商品，您可以在此　<a href="<%=request.getContextPath()%>/front/mall/onceAgainPutShopCart.action?orderId=<s:property value="order.id"/><%=addUrl %>" target="_blank">将商品放入购物车</a>
	    <!--如需再次购买订单中的商品，您可以在此　<a href="~/front/onceAgainPutShopCart.action?orderId=<s:property value="order.id"/>">将商品放入购物车</a>-->
	    </div>
   </DIV>
</DIV>
</DIV>
