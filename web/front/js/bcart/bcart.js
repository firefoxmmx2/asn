var bcart={
	...
	urls: {
			showProduct: "~/front/platform/mall/product/displayDetailProduct?productId={id}",
			listCart: "~/front/platform/mall/shoppingCart/listCart",
			changeQuantity: "~/front/platform/mall/shoppingCart/setQuantity",
			canChangeQuantity: "~/front/platform/mall/shoppingCart/canSetQuantity",
			clearCart: "~/front/platform/mall/test/xxx",
			addItem: "~/front/platform/mall/shoppingCart/toShoppingCart"
		},
		defaultWidth: 740,
		defaultHeight: 250,
		ui:null,
		templates:{
			item: "<div class='col cartItem' itemId='{itemId}' supplierId='{supplierId}'><div class='roc'>"+
				"<div class='col cartImage'><a target='_blank' href='{detailUrl}'><img src={image} border=0 width=80 height=80 /></a></div>"+
...
bcart.doCheckOut=function(){
	//alert("mytemplate>>" + mytemplate);
	window.location.href=butil.fullPath("~/front/platform/mall/toConfirmInfoPage.action?template="+mytemplate);
};
...
// 添加新项目
bcart.addItem=function(productId, quantity, supplierId, template){
	if(typeof(quantity)=="undefined") quantity=1;
	if(quantity=="") quantity=1;
	// 调用添加项目的Url
	//alert("template>>" + template);
	mytemplate = template;
	butil.getUrl(bcart.urls.addItem+"?template="+template, {productId: productId, count: quantity, supplierId: supplierId}); // TODO 提供参数
	bcart.showCart();
};
