 ...
 * 
 * Modified by Ken Liu Kunyun on March 9, 2010
 */ 
(function($, undefined){
		...
		var
			...
			.bind('focus.autoComplete', function(event, flag){
				if (!Active || 
					...
					LastEvent[$.expando + '_autoComplete_enter'])
						return TRUE;
				// prevent from show options after clicking option according to flag 
				if(flag === $.expando + '_autoCompleteClick') {
					$input.attr("justClicked", "yes");
					return true;
				}else if($input.attr("justClicked")=="yes"){
					$input.attr("justClicked", "no");
					return false;
				}
				// Store event
				LastEvent = event;
			...
			function userSuppliedData(event, settings, cache, backSpace){
				var list = [], // Result list
					...
					// Only compile regex if needed
					regex = fn ? undefined : new RegExp(cache.val=="" ? ".*" : cache.val.regexEscape(), 'i'), 
			...
			function loadResults(event, list, settings, cache, backSpace){
				...
				.bind('click.autoComplete', function(event){
					liData = $li.data(settings.dataName);
					if(typeof(liData)=="undefined" || liData==null) liData={value:$li.text()};
					...
					// Refocus the input box and pass flag to prevent inner focus events
					$input.trigger('focus', [$.expando + '_autoCompleteClick']);
					$ul.hide(event);
					autoFill(undefined);
					if (settings.onSelect) settings.onSelect.apply(self, settings.backwardsCompatible ? 
						[liData, $li, $ul, event] : [event, {data: liData, li: $li, ul: $ul}]);
...
// attach auto complete event
$(document).ready(function(){
	var inputs=$("input[autoCompleteOptions]");
	for(var i=0; i<inputs.length; i++ ) {
		var elem=inputs[i];
		var options=unescape($(elem).attr("autoCompleteOptions")).replaceToken("；", ";").split(";");
		$(elem).autoComplete({
			dataSupply: options
		});
	}
