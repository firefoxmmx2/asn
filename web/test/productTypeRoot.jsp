...
.category-item-fashion  {height:160px;}/*二级分类高度*/
.category-item-fashion h4 {height:160px;padding-top:15px;}/*二级分类高度*/
...
/*category-item*/
.category-item{overflow:hidden;float:left;*float:none;width:300px;border-bottom:1px dashed #ccc;}/*div宽度*/
.category-item h4{float:left;width:20px;padding-left:4px;padding-right:4px;background:#d8f0ce;border-right:1px solid #95d380;color:#000000;font-size:14px;text-align:center;line-height:20px;}/*一级分类背景样式*/
.category-item h4 a{color:#1f6f00!important;}/*一级分类文字样式*/
.category-item h4 a:hover{color:#f60!important;}
.category-item ul{float:left;width:270px;}/*div ul宽度*/
.category-item li{width:95%;float:left;height:20px;text-overflow: ellipsis;overflow: hidden;white-space: nowrap; margin:2px 0 1px;}/*三级分类样式*/
...
<h4><a href="<%=url0 %>&type=<%=item1.getPtcode() %>"><%=item1.getPttypeName() %> </a></h4>
<!--  2级分类  -->
...
<ul>
	...
	<h5>
	<a
		href="<%=url0 %>&type=<%=item2.getPtcode() %>" class="wz_2"><%=item2.getPttypeName() %> </a></h5>
	<div>
	<!-- 3级分类 -->
	<% for(ProductType item3 : (List<ProductType>)((Map)request.getAttribute("ptL")).get("ptThrL")) { %>
	<% if(item3.getPtparentid() == item2.getPtid()) { %>
	<a
		href="<%=url0 %>&type=<%=item3.getPtcode() %>" class="h"> <%=item3.getPttypeName() %> </a>
	<% } //if item3 %>
	<% } //for item3 %>  
	</div>
	</li>
