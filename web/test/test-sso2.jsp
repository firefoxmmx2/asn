...
<html>
  ...
  <body>
    This is my JSP page. <%if(isLogin){%>已经登录 [<%=loginUserPin%>]-[<%=loginUserName%>]    <a href="<%=sp.getProperty("sso_quit_url")%>?typecode=zxzx&backPath=<%=backPath%>" target="_blank">注销</a><%}else{%><a href="<%=sp.getProperty("sso_login_url")%>?backPath=<%=backPath%>">还没登录</a><%}%><br>
  </body>
</html>